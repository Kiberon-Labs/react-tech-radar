...
import {
	getColorScale,
	type QuadrantsConfigType,
	ThemeContext,
} from "../theme-context.js";
import { RadarContents } from "./Radar.style.js";
...
const calculateRadiusDiminish = (
	nrOfRings: number,
	radiusDiminishConstant: number,
) => {
	let max = 1;
...
export function Radar(props: RadarProps) {
	...
	const getRandomCoordinates = useCallback(
		(
			...
			const hasCollision = (results: RadarData[], coordinates: Coordinate) => {
				...
				for (const result of results) {
					if (
						...
						if (++collisionCount >= MAX_COLLISION_RETRY_COUNT) {
							console.log(
								`max collision retry limit reached: ${collisionCount}`,
							);
						}
						return true;
					}
			...
			const radiusArray = calculateRadiusDiminish(
				rings.length,
				radiusDiminishConstant,
			);

			const randomPosition = getPositionByQuadrant(radiusArray, rings, entry);
			...
			const data: Coordinate = polarToCartesian(r, theta);
		...
		[toleranceX, toleranceY, radiusDiminishConstant, width],
	);

	//given the ring and quadrant of a value,
	//calculates x and y coordinates
	const processRadarData = useCallback(
		(quadrants: string[], rings: string[], data: DataEntry[]) => {
			...
			for (const i in data) {
				...
				if (collisionCount < MAX_COLLISION_RETRY_COUNT) {
					collisionCount = coordinates.collisionCount ?? 0;
				}
		...
		[props.quadrants.length, getRandomCoordinates],
	);

	const points = useMemo(
		() => processRadarData(quadrants, rings, data),
		[quadrants, rings, data, processRadarData],
	);
